...
def test_autoresearch_cycle_runs_once_then_is_an_idempotent_noop(tmp_path, monkeypatch):
    ...
    assert runner_command[runner_command.index("--threads") + 1] == "8"
...
def test_deployment_payload_and_timer_are_pinned():
    ...
    assert (
        "--w0-release /etc/w09/deep03/releases/D3-W0-20260719-12.json"
        in service
    )
    assert (
        "--w1-release /etc/w09/deep03/releases/D3-W1-20260719-12.json"
        in service
    )
    assert (
...
def test_bounded_08_resource_and_packaging_contract_are_fixed():
    ...
    assert autoresearch.DEEP03_THREADS == 8
...
def _authority_files(
    ...
    *,
    ...
    operator_text = (
        ...
        "D3-W2A-2026-07-19.12 for the named release set."
    )
    ...
    w0_release_id = "D3-W0-2026-07-19.12"
    w1_release_id = "D3-W1-2026-07-19.12"
    ...
    authority = {
        ...
        "state": "ACTIVE",
        "release_id": "D3-W2A-2026-07-19.12",
...
def test_exact_release_authority_and_arm_bind_plan_runtime_and_input(tmp_path):
    ...
    assert result["release_id"] == "D3-W2A-2026-07-19.12"
