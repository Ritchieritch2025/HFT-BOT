...
def _order(oid, tk="KXBTC-25DEC31-B50", side="bid", rem=50000):
    return {"order_id": oid, "ticker": tk, "book_side": side,
...
def test_order_attr_drift_missed_partial_fill():
    ...
    assert "rem=20000" in drifts[0]["detail"] and "rem=50000" in drifts[0]["detail"]
...
def test_no_remedy_ever_says_resend():
    for cls, remedy in rec.REMEDY.items():
        low = remedy.lower()
        assert "resend" not in low or "not resend" in low or "never" in low, cls
        assert "retry" not in low, cls
...
def test_malformed_side_fails_closed():
    ...
    bad = {"resting_orders": [{"ticker": "X"}], "positions": []}  # no order_id
