...
read-loop/writer/control 3-thread soak against the mock; prod (read-only) manual smoke
(the demo env is no longer supported):
...
3. Measure batch-orderbooks token cost empirically in prod (read-only; script under
...
Gate: catalog bootstrap against prod (read-only) for a chosen series set; pagination
...
Gate: recorded prod (read-only) session (≥1 reconnect forced) replays checksum-identical.
...
2. Error-25 chaos drill in prod (read-only): over-subscribe conn B, throttle reads, verify
   I7 flow end-to-end.
...
2. Whether `update_subscription get_snapshot` seq-stamps the snapshot with the
   next sid seq or re-baselines — confirm against prod before finalizing
