...
Strategy (ground-up thesis, frozen):
  * both sides, price-improve +0.1c over best bid of each side,
    clamped to (opposite ask - 0.1c); post_only guards crossing.
  * zones by MARKET mid (model demoted): tte 10-30m all; 5-10m only
    |mid-50|>10; 2-5m only mid<20 or >80; <2m never (hard cancel).
...
"""
import asyncio, base64, collections, json, os, sys, time, uuid, urllib.request
...
SENTINEL_C = 15.0
MARGIN_C = float(os.environ.get("MM_MARGIN", "0.3"))  # dynamic-fair edge gate, cents
...
class S:
    ...
    last_replace = {}
    midhist = {}
    fills_cursor = int(time.time() * 1000)
...
def think():
    ...
    for mt, (ser, close_s, strike) in list(S.meta.items()):
        ...
        sigma = rp.sigma_from_ticks(ticks) or 0.0
        yb, nb = best(bk["y"]), best(bk["n"])
        if yb is None or nb is None: continue
        ya = 10000 - nb
        mid_c = (yb + ya) / 200.0
        p = rp.p_settle_above(ticks[-1], strike, sigma, tte)
        # --- dynamic fair: market level 10s ago + kernel-delta x RTI move ---
        # (wind-following: anchor jumps -> fair jumps NOW, book may lag)
        mh = S.midhist.setdefault(mt, collections.deque(maxlen=30))
        if not mh or now - mh[-1][0] >= 1.0:
            mh.append((now, mid_c, ticks[-1]))
        base = next(((t0,m0,r0) for t0,m0,r0 in mh if now-t0 >= 10), None)
        fair_c = mid_c
        if base is not None and sigma > 0 and tte > 60:
            import math as _m
            z = (ticks[-1]-strike)/(sigma*_m.sqrt(tte))
            beta = min(25.0, 100.0*_m.exp(-z*z/2)/_m.sqrt(2*_m.pi)/(sigma*_m.sqrt(tte)))
            fair_c = max(1.0, min(99.0, base[1] + beta*(ticks[-1]-base[2])))
        if p is not None and abs(100*p - mid_c) > SENTINEL_C:
            for side in ("bid","ask_no"):
                od = S.orders.pop((mt,side), None)
                if od: order_cancel(mt, side, od["id"])
            L.w({"ev":"SENTINEL_PAUSE","mt":mt,"fv":round(100*p,1),"mid":mid_c})
            continue
        ...
        want = {}
        y_px = q_px(yb, ya)
        n_px = q_px(10000-ya, 10000-yb)
        # wind gate: only rest on a side that is CHEAP vs dynamic fair.
        # RTI drops -> fair drops -> bid side turns rich -> cancel NOW
        # (without waiting for the book); mirror for the NO side.
        ...
        # A5 inventory skew: long YES -> only quote the NO side
        skew_block_bid = (np_["y"] - np_["n"]) >= MAX_NET
        skew_block_no  = (np_["n"] - np_["y"]) >= MAX_NET
        want["bid"] = ok and (mt,"bid") not in S.latch and y_px > 0 \
                      and room and edge_bid >= MARGIN_C and not skew_block_bid
        want["ask_no"] = ok and (mt,"ask_no") not in S.latch and n_px > 0 \
...
async def fills_task():
    while True:
        ...
        for f in (d.get("fills") or []):
            S.fills_cursor = max(S.fills_cursor, int(f.get("created_ts", 0)) + 1)
            L.w({"ev":"FILL","raw":{k:f.get(k) for k in ("ticker","side","count","price","created_ts")}})
            mt=f.get("ticker")
            side = "bid" if f.get("side") in ("yes","bid") else "ask_no"
            S.latch.add((mt, side))
            try:
                px = float(f.get("price",0)); n = float(f.get("count",0))
