...
## 5.5 流架构约束(2026-07-10 当日补充;PLAN_PRICING_MODEL /
## PLAN_RISK_KILLSWITCH 起草于本节之前,未含以下内容——
## 后续任何引擎/执行类 W 起草或计划修订时必须收编,审计对照)

1. **同一条流喂三种模式(结构性约束,不是偏好)。** 策略引擎只从
   进程内有序事件队列消费;灌流者开机接线一次:实盘/shadow =
   WS 解码线程,回测 = 仓库重放读取器。出口同理:提交队列后接
   真实 HTTP lane(live)/ 日志记录器(shadow)/ 成交模拟器
   (backtest),开机选定。**热循环内禁止任何模式判断分支。**
   推论即审计项:回测器与引擎若是两套决策逻辑 → 计划审计 REJECT
   ("回测过了、影子过了、实盘不一样"的鬼故事从结构上封死)。
2. **热路径零分配是会变红的测试,不是口号。** 引擎类 W 的验收
   必备:soak 测试挂分配计数钩子,稳态(预热后)热路径分配次数
   = 0,否则 FAIL。理由:分配的危害不在平均慢,在 p99 尖刺——
   行情最疯、最该撤单的毫秒里分配器打喷嚏 = 报价挂在旧世界被收割。
3. **重放即测速机。** 回测=同一热循环全速重放历史流,处理延迟
   p50/p99 顺手量出——生产代码即基准测试,不另写 benchmark。
   引擎 W 验收含一次全速重放的吞吐/延迟数字。
4. **引擎自记输入日志。** live/shadow 模式下引擎把消费过的每条
   事件异步落盘(热线程外写者,S5/E7)。用途:事故复现用引擎
   亲眼所见的流,不依赖采集进程的视角差;与仓库流互为对账。
5. **单 lane 吞吐数学(one-in-flight)。** 每条 HTTP lane 上
   一单等一回执,单 lane 吞吐 = 1/(签名+RTT);提高下单吞吐 =
   压单次延迟 × 增加并行 lane 数。lane 数是容量参数,进配置,
   不进代码常量。
