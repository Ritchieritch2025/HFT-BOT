...
- [P1-FIX v2 / F-4] writes are ENUMERATED (P7): the ONLY writable paths are
  `work/research/auto_research/<RUN_ID>/**` and `sandbox/research/**`;
  the canonical ledger `docs/research_notes/HYPOTHESIS_LEDGER.md` accepts
  APPEND-ONLY new hypothesis entries tagged with the source RUN_ID (merges
  and renumbering still go through the dedupe-before-number adjudication);
  all other `docs/**`, all config, and all production code are NOT writable
  by this mission;
...
GATE A — DATA PLANE
[P1-FIX v2 / gate rename, operator supersede 2026-07-14] The W05 gate for
this mission is W05_EXPLORATORY_READY (W05_ACCEPTED was revoked; formal
acceptance stays strict until PIPE-W03 ships). Prove W05_EXPLORATORY_READY
is returned and that a no-SSH consumer can:
- inventory;
- fetch;
- verify;
...
[P1-FIX v2 / F-1] HOLDOUT RESERVATION: confirmation-grade experiments may
only ever use data from days SEALED AFTER THIS MISSION'S START TIMESTAMP.
Every sealed day available at mission start is classified PRIOR-EXPOSED for
this mission and is recorded as such in PRIOR_EXPOSURE.json; this mission
must never open, peek at, or summary-scan a post-start sealed day except
through the formal confirmation protocol. The cost of future confirmation
data is therefore borne by new capture, never silently by re-used history.
Default analysis view:
  SEALED_CONFIRMATION only. [P1-FIX v2 / GATE A amendment, operator
  supersede 2026-07-14] While PIPE-W03 (capture-quality assessor) is
  pending, releases whose ONLY downgrade reason is the W03-pending
  capture-quality status carry the explicit tier
  SEALED_PENDING_QUALITY_ASSESSMENT and ARE admitted to the default
  analysis view for EXPLORATORY (Track A) work, with the tier banner
  mandatory on every artifact and NO freeze/verdict/promotion/
  live-candidate claims. VERDICT-grade claims remain SEALED_CONFIRMATION
  only, after the assessor exists and backfills.
...
[P1-FIX v2 / F-2] LOG-ODDS DISCIPLINE (GUARDRAILS Q1, constitutional MUST):
every volatility, skew, spread, drift, toxicity or other strategy-relevant
quantity over probability contracts is computed in LOG-ODDS SPACE; mapping
back to cents happens only at the presentation/execution edge. Price-space
math on probability contracts is a reject.
...
A. DATA EVIDENCE
- SEALED_CONFIRMATION
- SEALED_PENDING_QUALITY_ASSESSMENT [P1-FIX v2: W03-pending-only downgrade;
  exploratory-admissible with banner, never VERDICT-grade]
...
[P1-FIX v2 / F-3] SESSION BOUNDARIES AND RESUME PROTOCOL: this mission
spans multiple sessions. EVERY session ends with the mandatory CLAUDE.md
exit ritual (commit + SESSION_LOG entry + docs mirror) — nothing in this
mission, including this completion clause, exempts it. Resumption uses the
SAME RUN_ID; a resuming session MUST re-verify the §2 identity manifest
before appending to TRIAL_REGISTRY, and appends a resume record (session id,
UTC time, verified identities) to RUN_MANIFEST.json.
