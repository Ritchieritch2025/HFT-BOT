...
## 附 7：迁移前延迟基准五层 + 对照规矩（操作员 2026-07-08，W-LAT-BENCH）

迁移只改网络层，故基准须五层同测方能归因：① WS RTT（+p99.9+抖动，操作员
补：万分位尾巴和稳定性比均值更关键）② 交易所→本机单向延迟 ③ 解码 ns/条
（对照组，应不变）④ 端到端全链 ⑤ 时钟 offset（最易忽略：时钟偏则所有延迟
数字失真，迁移后必同测）。非延迟稳定性对照：吞吐率/seq缺口率/重连率/drop。
方法：同活跃度时间窗对比（禁深夜比白天）；基准存结构化 JSON，迁移后一键
diff。playbook 3h（W-LAT-BENCH）迁移前跑，W-A5 EC2 复跑出收益报告。
