...
- max 3.6-5ms 的尾部**推测**为 Mac 调度抖动(非专核、非实时优先级;未做
  亲和性实验验证),EC2 部署时另测(见 §5 Tier 3)。
- 决策环节旁证(此前已实测,引用):WS decode+book apply = 281.9ns/msg
  (bench_ws_decode,SESSION_LOG **2026-07-07 W5 条目**)。
...
| 合并 w1-w4(自检补算) | /exchange/status | 796 | 31.2 | 34.7 | 38.4 | 53.7 | 94.7 |
| 合并 w1-w4(自检补算) | /markets?limit=1 | 800 | 34.2 | 38.7 | 43.0 | 60.8 | 114.0 |
...
p99=36.3ms——与 Python 探针差 <1ms(单次对比),探针自身开销可忽略。
**时段覆盖(2026-07-16 自检时实算,替换初稿的未验证说法):**本次四窗
跨 15:24-17:16Z,未含 UTC 0-3;但网络层基线
work/latency_baseline/samples.ndjson(launchd 每 5 分钟一批,07-09 起,
自检时 2,189 条 ok 批、持续增长)**覆盖全部 24 个 UTC 小时**,实算结果:
全量 warm_p50 中位 = 34.5ms,按 UTC 小时分组的中位数漂移 = 2.6ms
(33.4→36.0),按天中位 32.1-37.1ms。即高峰时段无异常放大,鉴权层
数字可平移使用(鉴权层本身未在 0-3Z 实测,如实声明)。
...
在 us-east-1"是仓库内既有断言(apps/bench_rtt.cpp 注释,DOCS-ONLY 级,
未独立验证)——若成立,执行机放 us-east-1 还有余量(生产盒
catalog_sync 反推 <1ms 为旁证,SESSION_LOG:2132,非实测不入账)。
...
请求体传输(~300B,量级 µs)。规则(2026-07-16 自检时精确化口径):
`signed_post_rtt_p99_us = 合并(pooled)最差端点 p99 × 1.25 余量`,
测点=实际部署机。当前部署机未定 ⇒ 用 Mac 数字(比 EC2 悲观,方向安全,
符合 Q2)。Tier 4 实测后整段替换。

口径与棘轮(自检新增,防两类作弊/漂移):
- **口径 = 合并 p99,不用单窗 p99**:单窗 n≈200 的 p99 是第 ~198 位样本,
  统计噪声 ±2 个样本量级(实例:w3 markets 单窗 p99=83.7,合并即被
  稀释)。合并口径统计更稳,但会随加窗稀释尾部,故配棘轮。
- **棘轮:记账值只升不降。**新数据合并 p99 更高 ⇒ 上调;更低 ⇒ 保持,
  下调须操作员批准。当前账面 91000 基于记账时点(w1-w3)的 72.5ms;
  自检补算 w1-w4 合并 = 60.8ms(→76000)与单窗最差口径 83.7ms
  (→104600)均已知,**维持 91000**(介于两口径之间,棘轮不下调)。
...
- **Tier 2b(demo 下单/撤单正确性排练)= BLOCKED,操作员维持裁决
  ...
  安全层保持原样,未动。
- **Tier 3 EC2 侧曾于本 session 早段(约 15:1xZ,nc 探测 22 端口不通)受阻**(W09 停机、Mac 无控制面、生产采集机
