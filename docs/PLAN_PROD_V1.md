...
1. Create `tools.json` at repo root. One entry per binary/script:
   ```json
   {
     ...
     "needs": ["make"],           // or "mock_rest", "mini_redis", "creds"
...
## Phase P5 — Empirical verification campaign

**Goal**: burn down every "verify empirically, do not assume" item. The demo
exchange is no longer supported, so this runs against prod; every item here is
read-only (no orders), which is the only reason it is safe to point at prod.
Needs a real (prod) API key.

Tasks (record every result in `docs/KALSHI_RULEBOOK.md` + protocol doc):
1. `preflight` against prod (read-only): creds, clock skew, market-data parse.
...
3. `get_snapshot` seq semantics: scripted prod (read-only) session — subscribe, force
...
4. `ws_shadow` prod (read-only) soak: hours-long run, then the ≥24 h run with
...
strategy-visible corruption; prod (read-only) run shows measured event staleness ≪ the
...
Gate: tradingd in shadow mode on prod (read-only; WS feed from P6) runs with limits
...
Gate: full suite green. There is no demo exchange anymore, so live validation
places REAL orders on prod: `preflight --order` and `fill_test` on prod, then
tradingd live on prod with a throwaway 1-contract strategy (P9 test double)
placing post-only 1c bids — every order visible on the dashboard, canceled
cleanly, reconcile path exercised at least once by a forced timeout (the
forced-timeout case is proven deterministically against the mock; on prod it is
opportunistic). Treat every prod order as real money — floor the caps.
...
4. First real strategy lands behind this harness; acceptance = positive
   expectancy on recorded prod tapes + clean shadow run. (Strategy logic
...
across two runs; tradingd shadow on prod (read-only) shows strategy + PnL panels live.
...
4. Canary ladder — each step runs until its exit criteria hold, gated by
   ...
   c. (no demo `live` rehearsal exists anymore — the first live orders are on
      prod at the floored caps below; there is no risk-free dress rehearsal);
