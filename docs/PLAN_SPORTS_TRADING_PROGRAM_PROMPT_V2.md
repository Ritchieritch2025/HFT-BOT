...
The checkpoint candidate set is the exact BOOTSTRAP-0 pre-existing dirty set,
excluding:
...
- caches;
- generated bulk data;
- unknown artifacts not inspected and owned.
...
Archive this complete prompt, from `BEGIN CANONICAL PROMPT V2` through
`END CANONICAL PROMPT V2`, byte-for-byte to:
...
hash-sealed split manifest assigning complete root events to:
...
`tools.json` is the authoritative runnable registry. At v2 generation it
contained 144 entries; STP-P00 must reverify the count.
...
- `docs/PLAN_SPORTS_TRADING_PROGRAM_PROMPT_V2.md`;
- `docs/PLAN_SPORTS_TRADING_STATE.md`;
- append-only release/audit registration in
  `docs/PLAN_SPORTS_TRADING_DECISIONS.md`;
...
- all 144 registry entries covered;
...
Allowed only after W01 produces an evidence commit.

Audit writes are limited to:

- `docs/plan_audits/sports_trading_program/STP_P00_INDEPENDENT_AUDIT.md`;
- `docs/PLAN_SPORTS_TRADING_STATE.md`;
- append-only audit status in `docs/PLAN_SPORTS_TRADING_DECISIONS.md`;
- `docs/SESSION_LOG.md`.
...
6. A fresh independent session may then execute STP-P00-AUD01.
