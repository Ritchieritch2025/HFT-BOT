...
def taker_fee_usd(
    price: Decimal, count: Decimal = Decimal("1")
) -> Decimal:
    """Exact centicent fee from an exact fixed-point dollar price."""
    p = price
    c = count
    position_cost = p * c
...
def complementary_touch(
    ...
    if first_side == "bid":
        ...
        if yb is None or depth is None:
            return None
        return (
            Decimal("1") - Decimal(str(yb)) / Decimal("10000"),
            float(depth),
        )
    if first_side == "ask_no":
        ...
        if ya is None or depth is None:
            return None
        return Decimal(str(ya)) / Decimal("10000"), float(depth)
    return None
...
def main() -> None:
    ...
    for horizon in HORIZONS_S:
        ...
        for cycle in clean:
            ...
            if not Decimal("0") < touch_px < Decimal("1"):
                rejected["invalid_touch_price"] += 1
                continue
            if depth < 1.0:
                rejected["touch_depth_lt_1"] += 1
                continue
            entry_px = Decimal(str(first["price_dollars"]))
            touch_fee = taker_fee_usd(touch_px)
            touch_pnl = Decimal("1") - entry_px - touch_px - touch_fee
            conservative_px = min(
                Decimal("0.99"), touch_px + Decimal("0.01")
            )
            ...
            accepted.append({
                ...
                "touch_px": round(float(touch_px), 8),
                "touch_depth_ct": round(depth, 8),
                "touch_fee_usd": round(float(touch_fee), 8),
                "conservative_px": round(float(conservative_px), 8),
                "conservative_fee_usd": round(
                    float(conservative_fee), 8
                ),
                "capital_time_s": horizon,
                "touch_pnl_usd": round(float(touch_pnl), 8),
                "conservative_pnl_usd": round(
                    float(conservative_pnl), 8
                ),
            })
