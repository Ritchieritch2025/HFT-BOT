...
base binding, verifies caller-supplied Parquet bytes against every L1/L2 base
identity, and derives exact case-sensitive market universes in private local
scratch.  It performs no AWS operation and does not claim that the supplied
bytes were obtained by an exact-VersionId GET.
"""
...
def _normalize_family_inputs(
    ...
    for index, value in enumerate(values):
        identity, body = _exact_object(value, f"{family}_objects[{index}]")
        ...
        normalized.append((identity, body))
    normalized.sort(key=lambda item: item[0]["logical_key"])
    observed = [item[0] for item in normalized]
            ...
            try:
                ...
                for family in FAMILIES:
                    ...
                    family_receipts[family] = {
                        "family": family,
                        "base_family_set_sha256": base["families"][family][
                            "set_sha256"
                        ],
                        ...
                        "parquet_row_count": sum(
                            row["row_count"] for row in object_receipts
                        ),
                        ...
                        "object_receipt_set_sha256": canonical_sha256(
                            object_receipts
                        ),
                        "null_ts_utc_row_count": 0,
                        "cross_date_row_count": 0,
...
def _derive_universe_provenance(
    *,
    ...
    try:
        base = base_binding.build_base_binding(
