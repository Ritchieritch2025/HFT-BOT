...
DEFAULT_POSTFILL_V41_VALIDATOR_SQL_PATH = (
    ROOT
    / "tmp"
    / "crypto_mm_canary_20260726"
    / "round4"
...
def insert_postfill_v41_tables(
    connection: object,
    ...
    *,
    ...
    """Insert V4.1 rows only if the normative SQL batch gate is empty.

    The Python contract and DDL ``CHECK`` constraints are not the commit
    authority for cross-row economics.  This transaction inserts every
    normalized table, executes the separately pinned validator SQL, and
    rolls back if that query finds even one violation.  The validator never
    rewrites caller rows.
    """
    supplied_tables = set(rows_by_table)
    expected_tables = set(POSTFILL_V41_TABLES)
    if supplied_tables != expected_tables:
        raise Round4ContractError(
            "V4.1 table roster mismatch: "
            f"missing={sorted(expected_tables - supplied_tables)} "
            f"extra={sorted(supplied_tables - expected_tables)}"
        )
    validator_path = Path(validator_sql_path)
    validator_sql = validator_path.read_text()
    connection.execute("BEGIN TRANSACTION")
    try:
        row_count = 0
        for table in POSTFILL_V41_TABLES:
            table_columns = {
                str(info[1])
                for info in connection.execute(
                    f"PRAGMA table_info('{table}')"
            ...
            if not table_columns:
                raise Round4ContractError(
                    f"V4.1 sealed table is missing: {table}"
                )
            for row in rows_by_table[table]:
                columns = tuple(row.keys())
                unknown = set(columns) - table_columns
                missing = {
                    ...
                    if bool(info[3]) and info[4] is None
                } - set(columns)
                if unknown or missing:
                    raise Round4ContractError(
                        f"{table}: row/DDL field mismatch "
        ...
        violations = connection.execute(
            "SELECT violation_code, row_key, detail "
            "FROM postfill_v41_validation_violations "
            "ORDER BY violation_code, row_key"
        ).fetchall()
        if violations:
            preview = "; ".join(
                f"{code}:{key}:{detail}"
                for code, key, detail in violations[:5]
            )
            raise Round4ContractError(
                "V4.1 normative SQL validation failed: "
        ...
        raise
    return {
        "status": "POSTFILL_V41_SQL_GATE_VALID",
        "table_count": len(POSTFILL_V41_TABLES),
        "row_count": row_count,
