...
# Usage: ec2_s3_sync.sh hourly|daily
set -euo pipefail
MODE="${1:?usage: ec2_s3_sync.sh hourly|daily}"
cd "$HOME/hft-bot"
...
# raw: all day-dirs, excluding the hour being written right now
...
if [ "$MODE" = daily ]; then
  for d in facts dim catalog legacy_greed _meta; do
    [ -d "work/warehouse/$d" ] && \
...
echo "[ec2_s3_sync] $MODE sync complete $(date -u +%FT%TZ) (excluded hour $HH)"
